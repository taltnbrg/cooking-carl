	...
	return (
		<div>
			<InputGroup className="pt-1" onSubmit={(e) => e.preventDefault()} onKeyUp={handleKeyUp} onBlur={handleBlur}>
				<InputGroup.Text size="sm" className="rounded text-small" id="ingredient-input-label">{text.enter_ingredient[selectedLang]}</InputGroup.Text>
				<Form.Control
					type="text"
				...
				/>
				<Button className="rounded" variant="success" type="submit">{text.add[selectedLang]}</Button>
			</InputGroup>
			<IngredientDisplay ingredients={ingredients} handleDelete={handleIngredientDelete} />
			<div className="d-grid gap-2">
				<OverlayTrigger overlay={
					<Tooltip 
						show={ingredients.length<3} 
						id="tooltip-disabled">
